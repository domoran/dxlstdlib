...
if "%MASMPATH%"=="none" goto errMasm
    ...
    if exist "doorsOutput.obj" del "doorsOutput.obj"
    if exist "doorsOutput.exe" del "doorsOutput.exe"

    %MASMPATH%\bin\ml /c /coff /Fo"doorsOutput.obj" "doorsOutput.asm" 
    if errorlevel 1 goto errasm

    %MASMPATH%\bin\PoLink /SUBSYSTEM:CONSOLE "doorsOutput.obj" /OUT:"doorsOutput.exe"
    if errorlevel 1 goto errlink
    dir "doorsOutput.*"
    copy doorsOutput.exe ..
